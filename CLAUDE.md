...
devflow 是面向多 Agent 的 skill 集合，提供基于 mattpocock 方法论的开发工作流——从追问到发布。通过 SKILL.md 定义行为规范；用 `npx skills add` 从 GitHub 安装到当前宿主的 skills 目录。
...
安装后三个目录并排（宿主 skills 下的 `devflow*`），相对路径 `../devflow/references/` 仍有效。
